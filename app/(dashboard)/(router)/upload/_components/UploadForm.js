...
function UploadForm({ uploadBtnClick, isUploading, progress }) {
  ...
  const onFileSelect = (file) => {
    if (file && !allowedFileTypes.includes(file.type)) {
      setErrorMsg(
        "Unsupported file format. Please upload supported files (SVG, PNG, JPG, GIF, HEIC, DNG, PDF, ZIP, RAR, 7Z, TAR, GZIP, PPT, DOCS, CSV, video, audio)."
      );
      return;
    }
    if (file && file.size > 52428800) {
      setErrorMsg("Maximum File Upload Size is 50MB");
      return;
    }
    setErrorMsg(null);
    setFile(file);
